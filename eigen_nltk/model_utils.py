...
def freeze_bert(bert_model, freeze_layer_num):
    for layer in bert_model.layers:
        ...
        if fields[0] in ['Input', 'Embedding']:
...
class CRF(Layer):
    ...
    def __init__(self, units,
                 chain_kernel_constant_matrix=None,
                 learn_mode='join',
                 test_mode=None,
                 sparse_target=False,
                 use_boundary=True,
        ...
        self.chain_kernel_constant_matrix = chain_kernel_constant_matrix
        ...
        if self.chain_kernel_constant_matrix is not None:
            self.chain_initializer = initializers.constant(self.chain_kernel_constant_matrix)
            self.chain_trainable = False
        else:
            self.chain_initializer = initializers.get(chain_initializer)
            self.chain_trainable = True
    ...
    def build(self, input_shape):
        ...
        self.chain_kernel = self.add_weight(shape=(self.units, self.units),
                                            ...
                                            trainable=self.chain_trainable)
    ...
    def get_config(self):
        config = {
            ...
            'chain_kernel_constant_matrix': self.chain_kernel_constant_matrix,
...
def load_keras_bert_model(bert_keras_path, feature_only=True):
    ...
    layer_name_list = [e.name for e in bert_full_model.layers]

    if feature_only and tgt_layer_name in layer_name_list:
        layer_idx = layer_name_list.index(tgt_layer_name) - 1
        output_layer = bert_full_model.layers[layer_idx]
        feature_model = Model(bert_full_model.inputs[:2], output_layer.output)
        return feature_model
    return bert_full_model
...
# pick single vector from a sequence of vector with respect to index sequence
def pick_slice(seq_tuple):
    info_seq, idx_seq = seq_tuple
    info_seq = info_seq * idx_seq
    return K.sum(info_seq, axis=1)


VALID_FIT_GENERATOR_KWARGS = ["steps_per_epoch", "epochs", "verbose", "callbacks", "class_weight", "workers",
    ...
    if use_bert and (bert_ckpt_path or bert_keras_path):
        if bert_ckpt_path:
            bert_model = load_trained_model_from_checkpoint(os.path.join(bert_ckpt_path, "bert_config.json"),
                                                            ...
                                                            trainable=fine_tune_bert,
                                                            seq_len=max_len)
        else:
            bert_model = load_keras_bert_model(bert_keras_path)
            if not fine_tune_bert:
                freeze_model(bert_model)
        if freeze_layer_num > 0:
            bert_model = freeze_bert(bert_model, freeze_layer_num)

        word_feature = bert_model([words_input, seg_input])
    else:
